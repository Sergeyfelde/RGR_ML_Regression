...
# Создание и обучение модели StackingRegressor
...
stacking_results = evaluate_model(stacking_model, X_train, X_test, y_train, y_test, 'StackingRegressor')

# Сохранение результатов в CSV
stacking_results_df = pd.DataFrame({
    'Model': ['StackingRegressor'],
    'Train R2': [stacking_results['train_r2']],
    'Test R2': [stacking_results['test_r2']],
    'Train RMSE': [stacking_results['train_rmse']],
    'Test RMSE': [stacking_results['test_rmse']]
})
stacking_results_df.to_csv('results/stacking_results.csv', index=False)
print('Результаты StackingRegressor сохранены в results/stacking_results.csv')

# Сохранение модели с помощью pickle
with open('models/model_ml5.pkl', 'wb') as f:
    pickle.dump(stacking_model, f)
print('Модель StackingRegressor сохранена в models/model_ml5.pkl')
