...
cat_results = evaluate_model(cat_model, X_train, X_test, y_train, y_test, 'CatBoostRegressor')

# Сохранение результатов в CSV
cat_results_df = pd.DataFrame({
    'Model': ['CatBoostRegressor'],
    'Train R2': [cat_results['train_r2']],
    'Test R2': [cat_results['test_r2']],
    'Train RMSE': [cat_results['train_rmse']],
    'Test RMSE': [cat_results['test_rmse']]
})
cat_results_df.to_csv('results/catboost_results.csv', index=False)
print('Результаты CatBoostRegressor сохранены в results/catboost_results.csv')

# Сохранение модели с помощью метода save_model
cat_model.save_model('models/model_ml3.cbm')
print('Модель CatBoostRegressor сохранена в models/model_ml3.cbm')
